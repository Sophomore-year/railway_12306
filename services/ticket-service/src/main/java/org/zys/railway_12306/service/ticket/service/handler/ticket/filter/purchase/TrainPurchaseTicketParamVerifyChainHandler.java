...
import org.zys.railway_12306.framework.starter.common.toolkit.EnvironmentUtil;
import org.zys.railway_12306.framework.starter.convention.exception.ClientException;
import org.zys.railway_12306.service.ticket.constant.Railway12306Constant;
...
import static org.zys.railway_12306.service.ticket.constant.RedisKeyConstant.TRAIN_STATION_STOPOVER_DETAIL;
        ...
        String trainStationStopoverDetailStr = distributedCache.safeGet(
                ...
                () -> {
                    LambdaQueryWrapper<TrainStation> queryWrapper = Wrappers.lambdaQuery(TrainStation.class)
                            .eq(TrainStation::getTrainId, requestParam.getTrainId())
                            .select(TrainStation::getDeparture);
                    List<TrainStation> actualTrainStationList = trainStationMapper.selectList(queryWrapper);
                    return CollUtil.isNotEmpty(actualTrainStationList) ? JSON.toJSONString(actualTrainStationList) : null;
                },
                Railway12306Constant.ADVANCE_TICKET_DAY,
                TimeUnit.DAYS
        );
        List<TrainStation> trainList = JSON.parseArray(trainStationStopoverDetailStr, TrainStation.class);
        boolean validateStation = validateStation(
                trainList.stream().map(TrainStation::getDeparture).toList(),
